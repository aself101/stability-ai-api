...
[![Tests](https://img.shields.io/badge/tests-555%20passing-brightgreen)](test/)
...
Requires **Node.js 22 or later** (native `fetch`, `FormData` and `Blob`).
...
- **Comprehensive Testing** - 555 tests, 94.3% line coverage (api.ts 97.3%, cli-helpers.ts 100%, config.ts 93.8%, http.ts 89.7%, utils.ts 92.3%; measured at 1.0.1, 2026-09-22), no network access beyond local test servers; a spec-drift check against the live API runs in CI
...
- IPv6 addresses that embed an IPv4 address — mapped (`::ffff:…`, dotted or hex), translated, NAT64 (`64:ff9b::/96`), IPv4-compatible, 6to4 (`2002::/16`), Teredo (`2001::/32`), ISATAP — are judged by the IPv4 they route to; deprecated site-local `fec0::/10` is blocked
...
- API requests follow no redirects, and any redirect that changes origin drops credential headers
...
- [`openai-image-api`](https://github.com/aself101/openai-image-api) - OpenAI API wrapper for DALL-E and GPT Image generation
