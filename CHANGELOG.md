...
semantic-release from commit subjects and are kept as they were, except that their
heading levels were normalised (one `#` title, one `## [Unreleased]`) so changelog
parsers read the file as one document.
...
- **`--log-level` in any other case, or an unknown level, silenced the CLI entirely** —
  ...
  CLI flag); errors and warnings go to **stderr**. The CLI flag accepts exactly the four
  documented levels (`error`, `warn`, `info`, `debug`); winston's `http`, `verbose` and
  `silly` are rejected, so `--help`, the README and the error message list the same set.
  The library's `setLogLevel` still takes any winston level. Found by dx-validator.
- **A batch that failed part-way exited without saying what had been saved.** `sai generate`
  with several `--prompt`s now prints `Batch stopped: N of M saved; prompt N+1 failed; K
  not submitted.` before exiting 1. Found by dx-validator.
- **A rejected key's message dropped where keys are found.** The 401 message is now
  `Authentication failed. Check your API key (keys: https://platform.stability.ai/account/keys).`
  Code matching the old exact string must be updated. Found by dx-validator.
...
<!-- 0.1.0–0.1.4 were hand-written before semantic-release took over; the file's
original title block and its empty [Unreleased] heading stood here. -->
